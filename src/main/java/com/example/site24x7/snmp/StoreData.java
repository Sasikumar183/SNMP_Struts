...
public class StoreData {
   ...
   public static void fetchData() throws IOException {
        ...
        Map<Integer, Long> previousInTraffic = new HashMap<>();
        Map<Integer, Long> previousOutTraffic = new HashMap<>();
        Map<Integer, Integer> previousInErrors = new HashMap<>();
        Map<Integer, Integer> previousOutErrors = new HashMap<>();
        Map<Integer, Integer> previousInDiscards = new HashMap<>();
        Map<Integer, Integer> previousOutDiscards = new HashMap<>();
        Map<String, Integer> primeIdMap = new HashMap<>();
            ...
            try (PreparedStatement interfaceStmt = con.prepareStatement(interfaceQuery);
                ...
                for (int i = 0; i < dataArray.length(); i++) {
                    ...
                    if (!previousInTraffic.containsKey(interfaceId) || !previousOutTraffic.containsKey(interfaceId)
                        ...
                        int deltaInDiscards = (inDiscards >= prevInDiscards) ? (inDiscards - prevInDiscards)
                                : inDiscards;
                        int deltaOutDiscards = (outDiscards >= prevOutDiscards) ? (outDiscards - prevOutDiscards)
                                : outDiscards;
